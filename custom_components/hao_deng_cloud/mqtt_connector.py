...
class MqttConnector:
    ...
    def set_color(self, deviceId: int, red: int, green: int, blue: int):
        ...
        while red + green + blue > 630:
            ...
            green = green - 1
            blue = blue - 1
        _LOGGER.info("SET_COLOR Red: %s Green %s Blue %s", red, green, blue)
    ...
    def turn_off(self, deviceId: int):
        """Turn the light off."""
    ...
    def set_color_temp(self, deviceId: int, color_temp: int):
        ...
        if 2000 <= color_temp <= 6535:
            ...
            payload = MqttLightPayload(deviceId, "E2", f"0562{hex_value}640000000200")
            payloadJson = json.dumps(payload.__dict__)
            self.client.publish(
                f"/{self.software.productKey}/{self.software.deviceName}/control",
                payloadJson,
    ...
    def convert_notification_data_to_rgb(self, data: str) -> tuple[int, int, int]:
        _LOGGER.info("Recieved Light Data: %s", data)
        hue = data[6:8]
